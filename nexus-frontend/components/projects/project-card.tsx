...
export function ProjectCard({ project, onEdit, onDelete, isLoading }: ProjectCardProps) {
  ...
  return (
    <div className="bg-[#111113] border border-[rgba(255,255,255,0.07)] rounded-xl p-5 hover:border-[rgba(255,255,255,0.14)] hover:-translate-y-0.5 transition-all duration-150">
      {/* Title */}
      <h3 className="text-[15px] font-semibold text-white mb-2" style={{ fontFamily: 'Geist, sans-serif', fontWeight: 600 }}>
        {project.name}
      </h3>
      ...
      {/* Action buttons */}
      <div className="flex flex-col gap-2 mt-4 pt-4 border-t border-[rgba(255,255,255,0.06)]">
        <button
          onClick={() => router.push(`/projects/${project.id}/tasks`)}
          disabled={isLoading}
          className="w-full px-3 py-1.5 text-[12px] font-medium text-white bg-[#6c5ce7] rounded-[6px] hover:bg-[#7c6dfa] transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          View Tasks
        </button>
